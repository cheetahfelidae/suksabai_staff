...
var ls = {
    url: "guest-liveSearch/ajax/process_livesearch.php",
    form_id: "#ls_form",
...
select_result = function() {
    'use strict';
    var firstName = $( query.selected_row ).find( 'td' ).eq( ls.select_column_index ).html(),
        lastName = $( query.selected_row ).find( 'td' ).eq( ls.select_column_index + 1 ).html();
    query.val( "" );
    guestForm.retrieveGuests( firstName, lastName );
    hide_result();
};
/*
...
function adjust_result_position() {
    ...
    $( result ).css( {
        left: query.position().left + 1,
        width: query.outerWidth() - 2
    } );
}
